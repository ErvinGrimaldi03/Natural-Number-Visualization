...
if __name__ == '__main__':
    ...
    # save result
    np.save(f'map_data_3d_{N}_{n_neighbors}_{str(min_dist).replace(".", "")}', map_data)


    def plot():
        fig = plt.figure(figsize=(12, 12))
        fig.patch.set_facecolor('black')
        plt.scatter(map_data[:, 0], map_data[:, 1], s=0.005, c=np.arange(N), cmap='magma')
        plt.axis('off')
        plt.savefig(f"number_map{N}.png", dpi=2000, facecolor="black")
    plot()